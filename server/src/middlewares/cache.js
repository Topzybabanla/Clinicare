...
export const cacheMiddleware =
  ...
  async (req, res, next) => {
    ...
    try {
      const cacheData = cache.get(cacheKey); //retrieve data associated with cachedKey
      if (cacheData) {
        console.log(`Cache key found for: ${cacheKey}`);
        return res.json(cacheData); //send saved response back to client
      }
      ...
      res.json = function (data) {
        ...
        return originalJSON.call(this, data); //originalJSON is the res.json first saved, the call is used to invoke the method so that the original res.json is seen with the help of the this keyword, ensuring data can be properly passed to the original json
      };
...
export const clearCache =
  ...
  (req, res, next) => {
    ...
    console.log(
      `cleared ${matchingKeys.length} cache entris for ${
