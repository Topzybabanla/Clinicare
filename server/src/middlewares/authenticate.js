...
export const verifyAuth = tryCatchFn(async (req, res, next) => {
  ...
  let token;
  //checking for our token in the request headers object and ensuring it starts with the word bearer signature word ensuring its jwt type token
  if (
  ...
  //check if a user exists withour decoded id
