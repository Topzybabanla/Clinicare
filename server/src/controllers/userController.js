...
export const register = tryCatchFn(async (req, res, next) => {
  ...
  //req.validatedData is the data that has passed zod's validation recieved from the req.body
  const user = await userService.register(req.body, next);
  if (!user) return;
  //handle accessToken generation - we send the user to our createSendToken which extracts the id form jwt to sign
...
export const authenticateUser = tryCatchFn(async (req, res, next) => {
  ...
  const user = await userService.authenticatedUser(userId, next);
