...
// middlewares - functions that have access to the request and response obj and can perform any task specified. - execute a piece of code, make changes to the req or res obj, call the next handler function. It basically helps to add and reuse functions across the approutes and endpoints. The flow -
...
const connectDb = async () => {
  const connectionOptions = {
    // env files in node when reading must beging with process.env
    ...
    retryWrites: true, //enables automatic of some writes operations like insert or update a document
    retryReads: true, //enables automatic retry of read operations
    maxPoolSize: 50, //maximum number of connection in the mongodb connection pool, helps to manage concurrent request
  ...
  try {
    ...
    process.exit(1); //exit the process, 1 usually indictates error/failure
  }
};
...
process.on("uncaughtException", (err) => {
  console.error("UNCAUGHT EXECEPTION! shutting down...");
...
const startServer = async () => {
  try {
    ...
    process.on("unhandledRejection", (err) => {
      ...
      server.close(() => {
        console.log("Process terminated due to unhandle rejection");
        process.exit(1);
      });
    });
    // handle graceful shutdown
